...
import { FileSpec } from './types.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

export async function createFilesInteractive(): Promise<void> {
			...
			if (files.length === 0) {
				...
				files.forEach((file, index) => {
					console.log(
						...
						chalk.gray(`(${file.content.length} bytes)`)
					);
				});
				console.log();
			}
...
export async function createFilesFromConfig(configFile: string): Promise<void> {
	try {
		...
		const files: FileSpec[] = config.files.map((file: any) => ({
...
async function createFiles(files: FileSpec[]): Promise<void> {
	...
	for (const file of files) {
		try {
			...
			throw new Error(
				`Failed to create file ${file.name}: ${error instanceof Error ? error.message : String(error)}`
			);
		}
	}
}
